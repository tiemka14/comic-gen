...
class LoRATrainer:
    ...
    def prepare_dataset(self):
        ...
        val_dir = f"{self.dataset_path}/validation"
        ...
        for i, filename in enumerate(train_files):
            ...
            caption_path = os.path.join(train_dir, f"{Path(filename).stem}.txt")
        ...
        for i, filename in enumerate(val_files):
            ...
            caption_path = os.path.join(val_dir, f"{Path(filename).stem}.txt")
    ...
    def verify_dataset_structure(self):
        ...
        # Check if directories contain images (no repetition subdirectories)
        ...
        if not val_images:
            print(f"❌ No images found in validation directory: {val_dir}")
            return False
...
"""
        ...
        config_path = f"./configs/{self.comic_name}_dataset_config.toml"
        with open(config_path, 'w') as f:
            f.write(toml_config)
        ...
        print(f"🔧 Using TOML format for Kohya SS compatibility")
    ...
    def start_training_simple(self, config_path: str):
        ...
        training_cmd = [
            ...
            "--network_train_text_encoder",
            "--text_encoder_lr=1e-5",
            "--token string 'Stalwart'",
